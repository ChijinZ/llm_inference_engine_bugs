...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('data/add_pr_status.log'),
...
def process_bug_results():
    ...
    try:
        with open('data/merged_bug_results.json', 'r', encoding='utf-8') as f:
            bug_results = json.load(f)
    except FileNotFoundError:
        logger.error("data/merged_bug_results.json not found")
        return
    except json.JSONDecodeError as e:
        logger.error(f"Error parsing data/merged_bug_results.json: {e}")
        return
    ...
    logger.info("Saving results to data/final_bug_results.json...")
    try:
        with open('data/final_bug_results.json', 'w', encoding='utf-8') as f:
