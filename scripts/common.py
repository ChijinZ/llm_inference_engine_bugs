...
class GitHubAPIClient:
    ...
    def api_graphql(self, query: str, variables: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """
        Execute a GitHub GraphQL query with retries and rate-limit handling.
        Requires authentication (GITHUB_TOKEN); returns parsed JSON or None.
        """
        if not self.github_token:
            logger.error("GitHub GraphQL requires authentication; set GITHUB_TOKEN.")
            return None
        ...
        while True:
            ...
            try:
                ...
                response = self.session.post(self.graphql_url, json=payload, headers=headers, timeout=60)
                self.api_calls_made += 1
                self.check_rate_limit(response)
                ...
                if response.status_code == 200:
                    ...
                    logger.warning(f"GraphQL returned errors: {data.get('errors')}")
                    if attempt >= 5:
                        return None
                    ...
                    logger.warning(f"Rate limited ({response.status_code}) for GraphQL, waiting {wait_time}s before retry {attempt}")
                    time.sleep(wait_time)
                    continue
                else:
                    if attempt >= 5:
                        logger.error(f"GraphQL HTTP {response.status_code} after {attempt} attempts; giving up")
                        return None
                    wait_time = attempt * 20
                    logger.warning(f"GraphQL HTTP {response.status_code}; waiting {wait_time}s before retry {attempt}/5")
                    time.sleep(wait_time)
                    continue

            except requests.exceptions.RequestException as e:
                if attempt >= 5:
                    logger.error(f"GraphQL request exception after {attempt} attempts: {e}")
                    return None
                wait_time = attempt * 20
                logger.warning(f"GraphQL request exception: {e}; waiting {wait_time}s before retry {attempt}/5")
                time.sleep(wait_time)
                continue
            except Exception as e:
                logger.debug(f"Unexpected error in GraphQL request: {e}")
                return None

    def graphql_list_issues(self, owner: str, repo: str, page_size: int = 100, after: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """List issues via GraphQL with cursor pagination."""
        query = """
        query($owner:String!, $repo:String!, $pageSize:Int!, $after:String) {
          repository(owner: $owner, name: $repo) {
            issues(first: $pageSize, after: $after, states: [OPEN, CLOSED]) {
              pageInfo { hasNextPage endCursor }
              nodes { databaseId number createdAt url }
            }
          }
        }
        """
        variables = {"owner": owner, "repo": repo, "pageSize": page_size, "after": after}
        return self.api_graphql(query, variables)

    def graphql_list_pull_requests(self, owner: str, repo: str, page_size: int = 100, after: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """List pull requests via GraphQL with cursor pagination."""
        query = """
        query($owner:String!, $repo:String!, $pageSize:Int!, $after:String) {
          repository(owner: $owner, name: $repo) {
            pullRequests(first: $pageSize, after: $after, states: [OPEN, CLOSED, MERGED]) {
              pageInfo { hasNextPage endCursor }
              nodes { databaseId number createdAt url }
            }
          }
    ...
    def graphql_get_issue_by_database_id(self, owner: str, repo: str, database_id: int) -> Optional[Dict[str, Any]]:
        """Fetch a single issue by databaseId via GraphQL."""
        query = """
        query($owner:String!, $repo:String!, $id:Int!) {
          repository(owner: $owner, name: $repo) {
            issue(databaseId: $id) { url createdAt number title }
          }
        }
        """
        variables = {"owner": owner, "repo": repo, "id": int(database_id)}
        data = self.api_graphql(query, variables)
        if not data:
            return None
        issue = (data.get('data', {})
                 .get('repository', {})
    ...
    def graphql_get_pull_request_by_database_id(self, owner: str, repo: str, database_id: int) -> Optional[Dict[str, Any]]:
        """Fetch a single pull request by databaseId via GraphQL."""
        query = """
        query($owner:String!, $repo:String!, $id:Int!) {
          repository(owner: $owner, name: $repo) {
            pullRequest(databaseId: $id) { url createdAt number title }
          }
        }
        """
        variables = {"owner": owner, "repo": repo, "id": int(database_id)}
